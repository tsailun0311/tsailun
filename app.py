...
app = Flask(__name__)
title = ["蔡倫魔方", "LBL", "CFOP CROSS", "CFOP F2L",
         "CFOP OLL", "CFOP PLL", "CFOP 六格觀察法", "找不到頁面!!"]


@app.route('/cross', methods=['GET'])
def cross():
    return render_template('cross.html', title=title[2])


@app.route('/f2l', methods=['GET'])
def f2l():
    return render_template('f2l.html', title=title[3])
...
def lbl():
    return render_template('lbl.html', title=title[1])


@app.route('/oll', methods=['GET'])
def oll():
    return render_template('oll.html', title=title[4])


@app.route('/pll', methods=['GET'])
def pll():
    return render_template('pll.html', title=title[5])


@app.route('/pll2', methods=['GET'])
def pll2():
    return render_template('pll2.html', title=title[6])
